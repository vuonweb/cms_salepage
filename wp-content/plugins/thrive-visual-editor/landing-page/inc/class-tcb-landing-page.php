...
if ( ! class_exists( 'TCB_Landing_Page' ) ) {
	...
	class TCB_Landing_Page extends TCB_Post {
		...
		/**
		 * Holds an array of TCB_Landing_Page instances
		 *
		 * @var array
		 */
		...
		protected function strip_head_css() {
			...
			$tcb_style_classes = array( 'tve_user_custom_style', 'tve_custom_style', 'tve_global_style', 'tve_global_variables' );
		...
		public function get_cloud_css_v2( $for_lightbox = false, $lb_suffix = '' ) {
			...
			$css = apply_filters( 'tcb_alter_cloud_css', $css, $this->cloud_template_data );
		...
		public function default_content( $default = 'blank_v2' ) {
			...
			return apply_filters( 'tcb_landing_page_default_content', $content );
		}
