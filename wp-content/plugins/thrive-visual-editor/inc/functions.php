...
function tve_load_global_styles() {
	$shared_styles = tve_get_shared_styles( '', '300' );
	$shared_styles = str_replace( '#tve_editor', '', $shared_styles );
...
function thrive_editor_admin_bar( $wp_admin_bar ) {
	...
	if ( is_admin_bar_showing() && ( is_single() || is_page() ) && tve_is_post_type_editable( get_post_type() ) && current_user_can( 'edit_post', $post_id ) ) {
		...
		if ( ! isset( $_GET[ TVE_EDITOR_FLAG ] ) ) {
			...
			$args        = array(
				'id'    => 'tve_button',
				'title' => '<span class="thrive-adminbar-icon"><img src="' . tve_editor_css() . '/images/thrive-architect-logo-white.png" style="width:1em;vertical-align:initial;"/></span>' . __( 'Edit with Thrive Architect', 'thrive-cb' ),
				'href'  => $editor_link,
				'meta'  => array(
...
function tcb_custom_editable_content() {
	...
	if ( ! $tcb_inactive && ! empty( $lp_template ) && defined( 'TVE_IN_ARCHITECT' ) ) {
		/**
	...
	if ( is_editor_page() && tve_is_post_type_editable( get_post_type( get_the_ID() ) ) ) {
		...
		if ( tve_tcb__license_activated() || apply_filters( 'tcb_skip_license_check', false ) ) {
			...
			if ( apply_filters( 'tcb_user_can_edit', true, $post_id ) ) {
				...
				tve_enqueue_script( 'tcb-froala', tve_editor_url( 'editor/js/dist/froala' . $js_suffix ), array( 'jquery' ) );
 ...
 * @return string
 */
function tve_get_shared_styles( $post_content = '', $for_media = '' ) {
	$output         = '';
	$shared_styles  = array();
	$button_styles  = get_option( 'tve_global_button_styles', array() );
	$section_styles = get_option( 'tve_global_section_styles', array() );
	$cb_styles      = get_option( 'tve_global_contentbox_styles', array() );

	$styles_types = apply_filters( 'tcb_get_extra_global_styles', array( $button_styles, $section_styles, $cb_styles ) );
	...
	foreach ( $styles_types as $style_type ) {
		...
		foreach ( $style_type as $identifier => $styles ) {
			...
			if ( ! empty( $post_content ) && ( strpos( $post_content, TVE_GLOBAL_STYLE_BUTTON_CLS_PREFIX . $identifier ) === false
			                                   && strpos( $post_content, TVE_GLOBAL_STYLE_SECTION_CLS_PREFIX . $identifier ) === false
			                                   && strpos( $post_content, TVE_GLOBAL_STYLE_CONTENTBOX_CLS_PREFIX . $identifier ) === false ) ) {
				continue;
			}
	...
	return sprintf( '<style type="text/css" class="tve_global_style">%s</style>', tve_prepare_global_variables_for_front( stripslashes( $output ) ) );
}
...
function tve_render_custom_menu( $attributes ) {
	$custom_menu_items     = array( 'Menu Item 1' => array(), 'Menu Item 2' => array( 'Menu Item 3', 'Menu Item 4' ), 'Menu Item 5' => array() );
	...
	$group_edit            = ! empty( $attributes['group_edit'] ) ? $attributes['group_edit'] : array();
	if ( ! empty( $group_edit ) ) {
		$GLOBALS['tve_menu_group_edit'] = $group_edit;
		add_filter( 'wp_nav_menu_items', 'tve_menu_add_li_data_css', 10, 2 );
	}

	$trigger_icon_sets = tcb_elements()->element_factory( 'menu' )->get_trigger_icons();

	if ( ! empty( $attributes['mobile_icon'] ) ) {
		$trigger_icons = $trigger_icon_sets[ $attributes['mobile_icon'] ];
	} else {
		$trigger_icons = reset( $trigger_icon_sets );
	}
	$trigger_icons_html = '<div class="thrv_wrapper thrv_icon tve-draggable tve-droppable tcb-icon-open tve-trigger-active" data-not-changeable="true">' . $trigger_icons['open'] . '</div><div class="thrv_wrapper thrv_icon tve-draggable tve-droppable tcb-icon-close"  data-not-changeable="true">' . $trigger_icons['close'] . '</div>';
	$custom_menu_html   = '<div class="thrive-shortcode-html tve-menu-custom-type thrive-shortcode-html-editable tve-custom-menu-type tve_clearfix " ' . $head_css_attr . '>

	<a' . $trigger_color . ' class="tve-m-trigger t_' . $attributes['dir'] . ' ' . $attributes['color'] . '" href="javascript:void(0)">' . $trigger_icons_html . '</a>';

	$custom_menu_html .= '<ul ' . $ul_custom_color . ' id="menu-main-menu" class="tve_w_menu tve-m-collapsed ' . $attributes['dir'] . ' ">';
	$icons            = tve_menu_custom_create_dropdown_icons( $attributes['dropdown_icon'] );

	foreach ( $custom_menu_items as $key => $values ) {
		if ( ! empty( $values ) ) {
			$custom_menu_html .= '<li id="tcb_custom_menu_' . tve_unique_id() . '" class="menu-item menu-item-type-post_type menu-item-object-page thrv_wrapper thrv-menu-item-element thrv-custom-menu-item menu-item-has-children">' .
			                     '<a href=""><span class="tve-disabled-text-inner">' . $key . '</span><span class="tve-disabled-text-inner tve-item-dropdown-trigger">' . $icons . '</span></a>';
			$custom_menu_html .= '<ul class="sub-menu tve_empty_dropzone">';
			foreach ( $values as $value ) {
				$custom_menu_html .= '<li  id="tcb_custom_menu_' . tve_unique_id() . '" class="menu-item menu-item-type-post_type menu-item-object-page thrv_wrapper thrv-menu-item-element thrv-custom-menu-item"><a href=""><span class="tve-disabled-text-inner">' . $value . '</span><span class="tve-disabled-text-inner tve-item-dropdown-trigger"></span></a></li>';
			}
			$custom_menu_html .= '</ul>';
			$custom_menu_html .= '</li>';
		} else {
			$custom_menu_html .= '<li  id="tcb_custom_menu_' . tve_unique_id() . '" class="menu-item menu-item-type-post_type menu-item-object-page thrv_wrapper thrv-menu-item-element thrv-custom-menu-item"><a href=""><span class="tve-disabled-text-inner">' . $key . '</span><span class="tve-disabled-text-inner tve-item-dropdown-trigger"></span></a></li>';
		}
	}
	$custom_menu_html .= '</ul></div>';

	return $custom_menu_html;
...
function tve_render_widget_menu( $attributes ) {
	...
	add_filter( 'wp_nav_menu_objects', 'tve_menu_custom_label_name', 10, 3 );
	...
	$trigger_icons_html = '<div class="thrv_wrapper thrv_icon tve-draggable tve-droppable tcb-icon-open tve-trigger-active" data-not-changeable="true">' . $trigger_icons['open'] . '</div><div class="thrv_wrapper thrv_icon tve-draggable tve-droppable tcb-icon-close"  data-not-changeable="true">' . $trigger_icons['close'] . '</div>';
	$menu_html          = '<div class="thrive-shortcode-html thrive-shortcode-html-editable tve_clearfix" ' . $head_css_attr . '><a' . $trigger_color . ' class="tve-m-trigger t_' . $attributes['dir'] . ' ' .
	                      $attributes['color'] . '" href="javascript:void(0)">' . $trigger_icons_html . '</a>' .
	                      wp_nav_menu( array(
		                      'echo'           => false,
		                      'menu'           => $menu_id,
		                      'container'      => false,
		                      ...
		                      'menu_class'     => 'tve_w_menu tve-m-collapsed ' . $attributes['dir'] . ' ' . ( ! empty( $attributes['font_class'] ) ? $attributes['font_class'] . ' ' : '' ) . $attributes['color'],
	                      ) ) . '</div>';

	/* clear out the global variable */
	unset( $GLOBALS['tve_menu_link_custom_color'], $GLOBALS['tve_menu_top_link_custom_color'], $GLOBALS['tve_menu_font_class'], $GLOBALS['tve_menu_top_link_custom_font_family'], $GLOBALS['tve_menu_group_edit'] );
	...
	remove_filter( 'wp_nav_menu_objects', 'tve_menu_custom_label_name' );

	return $menu_html;
}


function tve_menu_custom_label_name( $items ) {
	...
	foreach ( $items as $menu_item ) {
		...
		if ( in_array( 'menu-item-has-children', $menu_item->classes ) ) {
			$dropdown = '<span class="tve-disabled-text-inner tve-item-dropdown-trigger">' . $icons . '</span>';
		}
...
function tve_menu_custom_create_dropdown_icons( $style ) {
	...
	return '<svg xmlns="http://www.w3.org/2000/svg" class="tve-dropdown-icon-up" viewBox="0 0 320 512">' . $icon_styles[ $style ]['svg_up'] . '</svg>' . '<svg xmlns="http://www.w3.org/2000/svg" class="tve-dropdown-icon-down" viewBox="0 0 320 512">' . $icon_styles[ $style ]['svg_down'] . '</svg>';
}
...
function tve_widget_menu_li_classes( $classes ) {
	$classes [] = 'thrv_wrapper thrv-menu-item-element';
...
if ( ! function_exists( 'tve_frontend_enqueue_scripts' ) ) {
	...
	function tve_frontend_enqueue_scripts() {
		...
		if ( $theme_dependency === '1' && is_singular() ) {
