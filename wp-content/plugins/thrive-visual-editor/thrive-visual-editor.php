...
if ( is_admin() ) {
	/**
	 * Architect Product must be included only if Architect is active
	 */
	add_filter( 'tve_dash_installed_products', array( tcb_admin(), 'add_to_dashboard_list' ) );
}
...
function tve_ajax_landing_page_import() {
	...
	$is_post_type_allowed = in_array( get_post_type( $_POST['page_id'] ), apply_filters( 'tve_post_type_can_use_landing_page', array( 'page' ) ) );
